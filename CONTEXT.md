...
交易首次通过账单补录进入应用时记录的外部应用或渠道。作为不可变的溯源证据保留，即使其支付来源后续被修正。
_避免使用_：支付来源、条目来源

**条目来源 (Entry Origin)**：
交易进入应用的方式，例如手动录入、账单同步或重复合并；旧数据可能保留历史来源值。
_避免使用_：支付来源、资金账户
...
应用将用户主动导入的微信和支付宝页面可靠转换为待确认条目的精准程度。
_避免使用_：解析器准确率、通知准确率
...
**补录权限可用性 (Import Permission Availability)**：
用户发起补录时，无障碍授权与服务连接可用的程度。
_避免使用_：权限转化率、通知权限留存
