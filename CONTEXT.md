...
**原始导入来源 (Original Import Source)**：
交易首次通过自动识别或历史导入进入应用时记录的外部应用或渠道。新通用识别统一记录为“其他应用”；历史微信/支付宝值继续保留。
_避免使用_：支付来源、条目来源

**条目来源 (Entry Origin)**：
交易进入应用的方式，例如手动录入、无障碍自动识别或重复合并；旧数据可能保留通知捕获和账单同步来源值。
_避免使用_：支付来源、资金账户
...
个人中心区域的入口，用于账户同步、当前账本 CSV 导出以及所有账本加密备份的导出或导入。“本机数据删除”是该入口底部单独受保护的破坏性操作。
_避免使用_：账号管理、账单同步

**用户账号 (User Account)**：
用于身份验证、账户级云配置与正式账本同步的用户应用身份。
_避免使用_：设备 ID、个人资料

**公开账号 UUID (Public Account UUID)**：
后端为每个用户账号生成的不可变 UUID，用于账户管理页展示、复制和人工识别。它不同于数据库内部自增 `accountId`、安装 UUID 和可轮换的 Session Token，也不具备认证能力。
_避免使用_：Bearer Token、设备 ID、数据库主键

**账号管理 (Account Management)**：
个人中心区域的第一个入口，用于查看当前账号状态与公开账号 UUID、从本地模式以用户名、邮箱、手机号或已配置的微信身份登录/注册，以及登录后修改昵称/头像、补绑或换绑手机号/邮箱、管理微信身份和账号注销。
_避免使用_：个人资料、本地数据设置
...
后端已知晓的用户 Android 设备，用于账号安全、云配置及账户同步。
_避免使用_：客户端、安装实例

**云端配置 (Cloud Configuration)**：
由后端存储的非账本账号设置，例如同意状态和功能开关；它与账户同步保存的正式账本记录相互独立。
_避免使用_：云端账本、远程备份

**账号注销 (Account Deletion)**：
用户申请删除云端账号身份、登录标识、已注册设备、云端配置、AI 分类日志及全部云端同步账本数据的操作。本机账本保留并解除同步绑定。
_避免使用_：退出登录、卸载
...
用户通过已绑定并验证的手机号或邮箱验证码设置新密码，从而重新获取账号访问权的工作流。纯用户名账号未绑定联系方式时不能自助找回。
_避免使用_：短信验证码登录、客服重置
...
面向合规与隐私的用户可控入口，用于加密的本机应用故障排查事件。在 Debug 和 Release 中均可用，仅在 Debug 下默认开启；绝不上传日志，自动记账页面文字和交易字段不得写入，并始终脱敏身份凭证与密钥。
...
**导入准确率 (Import Accuracy)**：
应用将第三方应用当前活动窗口中的明确已完成交易可靠转换为待确认条目的精准程度。
_避免使用_：平台适配准确率、通知准确率
...
**捕获原因 (Capture Reason)**：
向用户展示的创建或匹配待确认条目的原因说明。新自动候选使用“支付结果自动捕获”；旧数据可保留通知捕获、账单同步或重复合并。
_避免使用_：调试原因、解析器追踪
...
**自动记账运行状态 (Automatic Bookkeeping Runtime Status)**：
用户意图开关、无障碍授权与服务连接组合后的状态：已关闭、等待授权、等待服务连接或正在监听。授权或连接中断不反写用户意图。
_避免使用_：补录权限、平台状态
...
**通用自动识别 (Generic Automatic Recognition)**：
用户开启后，基于 Assists 被动读取第三方应用当前活动窗口的受限可见文字，并只为满足完成态、唯一金额、明确方向和交易上下文的页面创建待确认候选。
_避免使用_：平台适配、OCR、自动操作
