...
- 已登录时，展示默认/HTTPS 微信头像、昵称、脱敏手机号和“手机号登录/微信登录”标签；根据凭据状态提供绑定微信、绑定手机号或合并、解绑微信。
- 合并确认展示当前与来源账号，明确“当前配置优先、来源 AI 日志删除、本机账本不变、来源云账号删除且不可自动撤销”，并要求输入“合并账号”。
- 解绑微信要求选择密码或短信验证，并说明解绑后保留的手机号登录方式；纯微信账号不显示可执行的解绑入口。
...
- 猫咪陪伴形象。
- 价值主张说明。
- “微信登录/注册”位于手机号登录和创建账号之前；无 AppID 时隐藏。
- 手机号登录与注册页底部提供相同微信入口。
- 本地模式入口。
...
- 短信验证码。
- 设置密码。
- 未绑定微信授权后先展示头像和昵称，可创建纯微信账号，或选择密码/短信绑定已有手机号账号。
- 纯微信账号在账号管理中可用专项短信新增未注册手机号并设置密码；已注册手机号进入明确的合并确认。
