...
待确认队列在导航顺序中排在第一位。自动记账开关、运行状态与无障碍授权入口位于“我的 → 自动记账”。
...
- 捕获时间。
- 解析字段。
- 新自动候选只显示规范化解析字段，不显示原始页面文字；历史条目仍按已有持久化证据兼容展示。
...
## 5. 自动记账 (Automatic Bookkeeping)

入口位于“我的 → 自动记账”，页面只包含：
- 总开关，默认关闭。
- 当前状态：`已关闭`、`等待无障碍授权`、`等待服务连接`、`正在监听`。
- 缺少授权时显示“打开无障碍设置”；服务断开不自动关闭总开关。
- 隐私说明：只在本机读取当前活动窗口中可见、非输入框文字；不截图、不操作其他应用、不保存或上传原始页面文字；识别结果只进入待确认。

页面不提供平台选择、手动补录、来源启动、进度步骤或 OCR 说明。
...
- 敏感异常内容默认显示为遮罩；用户再次确认后仅在当前页面会话显示。离页或进入后台立即重新遮罩，显示期间窗口启用 `FLAG_SECURE`。自动记账页面文字不进入诊断日志。
