...
> 历史规划：早期切片中的通知监听、连续观察和后台自动采集目标已退出当前产品基线；当前实现以根目录 README 和现行产品文档为准。
...
## Slice 16: 账户级离线优先账本同步

预期成果：
- 已验证账号可在多台设备间自动同步正式账本数据，离线记账不被网络或 Session 状态阻塞。

包含内容：
- 类型化同步契约、Backend 数据库 v7 与 Room v8。
- 持久化 outbox、记录版本、增量游标、删除墓碑、业务唯一键规范化和人工冲突。
- 首次告知、状态/重试/冲突 UI、账号切换事务和 HTTPS/局域网 HTTP 测试边界。

验证方式：
- Shared API、Backend、Room/Repository、WorkManager 与 Compose 自动化测试。
- 两台真实设备完成新增、编辑、离线冲突、删除/恢复、签出恢复、换账号和 Release HTTPS/HTTP 验收。
