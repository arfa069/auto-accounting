...
无障碍节点主链路和微信空节点页面的受限本地 OCR 兜底已实现；支付宝商户支付、支付宝转账、微信商户支付和微信转账成功页已通过真机验证。微信发送红包不存在独立成功页，已明确不通过付款发起页或聊天页推断成功。
...
- [ ] 支付宝商户扫码/碰一碰、转账，以及微信扫码支付、转账等受支持结果页至少各有一条脱敏真机证据；来源应用不存在独立结果页的场景明确记录为不支持。
