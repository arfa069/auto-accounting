...
- 账目新增和编辑表单只选择已有资金账户；新账户统一从独立资金账户页创建。
...
- 账目来源与生命周期元数据继续持久化，但当前账本 UI 不组合录入方式、生命周期、原始来源、原待确认 ID 或采集证据区域。
...
- [x] 自动确认优先保留已有账户 ID，否则只精确复用现有账户，不自动创建；账目表单只选择已有账户。
...
- [x] 来源与生命周期元数据继续持久化并进入备份，但账本界面不提供对应详情区域；编辑用户字段不得覆盖这些元数据。
...
4. 从独立页面新增、编辑和删除资金账户；确认账目新增/编辑页不显示“新建资金账户”，再用四类引用各验证一次删除失败提示，并确认历史账目支付来源未被批量修改。
5. 使用脱敏数据完成 V4 全量备份往返，并用 V2/V3 测试备份验证归入“默认账本”；损坏引用的备份不得改变当前数据。
6. 点击账目行确认直接进入编辑页；编辑页不显示来源与生命周期元数据，但保存用户字段、删除、最近删除和撤销均可用。
...
- UI 隐藏只改变组合层，不得删除来源元数据或削弱备份、去重和审计所需的持久化字段。
...
- `2026-07-17`：账目行改为直接进入共享编辑页，删除入口迁入编辑页；新增和编辑页均不再提供“新建资金账户”，来源与生命周期区域不再由账本 UI 组合。账本 Compose 专项测试、Android 全量单测和签名 Release 构建通过，Release 已覆盖安装到 USB 真机。
