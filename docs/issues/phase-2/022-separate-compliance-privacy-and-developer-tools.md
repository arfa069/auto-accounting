...
让所有用户（包括本地模式）能分别查阅合规材料，同时把 Debug-only 内测工具与用户控制的敏感诊断日志明确隔离。

## 范围

- “合规与隐私”二级页提供隐私政策、个人信息收集清单、第三方服务清单和权限说明四个独立入口。
- 每项打开各自完整内容页，并可在本地模式使用。
- 内测就绪检查、设备矩阵、权限留存和质量指标仅进入 Debug 构建的开发者工具；Release 构建和“我的”总览不显示开发者工具入口。
- “诊断日志”作为独立用户控制项在 Debug、Release 均显示于“合规与隐私”；Release 默认关闭并要求知情确认，不归入 Developer Tools。
...
- [x] Release 变体和“我的”总览不显示开发者工具、内测指标或设备矩阵。
- [x] Debug 变体可进入开发者工具，且不需要隐藏手势。
- [x] 合规入口不把商店审核说明当作普通用户内容展示。
- [x] Debug、Release 均显示独立诊断日志入口；Release 初始关闭并在开启前显示敏感记录说明。
...
2. 分别安装或检查 Debug 与 Release 构建，确认开发者工具只在 Debug 可见，而诊断日志入口在两种构建中均可见。
3. 确认 Release 诊断日志默认关闭，首次开启必须阅读并确认说明。

## 回滚或安全说明

- 开发者工具不得显示真实交易、手机号、令牌或未脱敏日志；敏感交易内容只能进入独立诊断页，且默认遮罩并受 `FLAG_SECURE` 保护。
...
- 2026-07-17：敏感诊断日志改为“合规与隐私”中的独立入口，取代未接入生产链路的内测日志占位；自动化验证覆盖 Release 入口可见且初始关闭、Developer Tools 仍不可见，以及 Debug 默认开启。Android 完整单测、Debug/Release 构建和 Release v2 签名校验通过；本轮未操作真机或查看真实诊断内容。
