...
本报告对 **bks** 项目进行了全面的代码质量、系统架构、测试覆盖率、安全合规性及维护性审查。
...
| :--- | :--- | :--- | :--- |
| **系统架构与解耦** | **优秀 (A)** | 规范的 Kotlin/Gradle 多模块设计，清晰的分层架构（Local-first, Shared API, Backend Ktor） | [MainActivity.kt](../../apps/android/src/main/java/com/bks/MainActivity.kt) 代码行数较长，职责较重 |
...
| **数据库与持久化** | **优秀 (A)** | Room v1~v7 Migration 逻辑严密，拥有自动化迁移演进测试 | [LocalLedgerRepository.kt](../../apps/android/src/main/java/com/bks/data/local/LocalLedgerRepository.kt) 类颗粒度较大 |
...
- **高覆盖率核心区（质量基石）**：
  - [data.local](../../apps/android/src/main/java/com/bks/data/local): **91%** 指令覆盖率，保障了 Room 数据的读写与版本演化稳定性。
  - [feature.dedupe](../../apps/android/src/main/java/com/bks/feature/dedupe): **95%** 指令覆盖率，去重算法测试极其严密。
  - [feature.categorization](../../apps/android/src/main/java/com/bks/feature/categorization): **95%** 指令覆盖率，智能分类与规则判定稳固。
  - [feature.ledger](../../apps/android/src/main/java/com/bks/feature/ledger): **89%** 指令覆盖率。

- **边缘与UI装配区（待加强）**：
  - [feature.capture](../../apps/android/src/main/java/com/bks/feature/capture): **66%** 覆盖率（涉及真实无障碍服务与通知监听器的系统环境依赖）。
  - [com.bks (Root/MainActivity)](../../apps/android/src/main/java/com/bks): **69%** 覆盖率。
...
1. **Local-First 数据隔离架构**：
   - 在 2026-07-22 审查快照中，账单数据、分类规则、证据库完全留存在 Android 端 Room 数据库中，后端仅负责账号身份、设备注册和 AI 分类代理；后续账户同步边界见文首说明。
...
3. **内置密钥扫描防御**：
   - 包含 [SecretScannerTest.kt](../../services/backend/src/test/kotlin/com/bks/backend/SecretScannerTest.kt)，防止硬编码 API Key、Bearer Token 或误将敏感配置文件提交至 Git 版本库。
...
- **文件路径**：[MainActivity.kt](../../apps/android/src/main/java/com/bks/MainActivity.kt) (1,093 行)
...
- **建议方案**：建议引入基于 Feature 的 AppState Holder / Controller（例如 `rememberBksAppState()`），并将具体的权限处理和配置逻辑抽离为独立的 Helper 类，保持 `MainActivity` 轻量干净。

### 2. Repository 集中度偏高
- **文件路径**：[LocalLedgerRepository.kt](../../apps/android/src/main/java/com/bks/data/local/LocalLedgerRepository.kt) (~650 行)
