...
- Coil / OkHttp 开源组件：仅用于从 HTTPS 头像 URL 加载微信头像；组件本身不提供遥测或云端存储。图片使用独立、上限 10 MB 的本机缓存，并在 URL 更新、退出、解绑、Session 失效或本机数据删除时清理。
- 短信服务商。
...
- **微信身份与资料**：绑定期间保存并在每次成功授权后刷新；解绑微信或账号注销完成后删除。设备端头像缓存会在 URL 更新、退出、解绑、Session 失效或本机数据删除时清理。
...
账号凭据与 Session 处理：
- Android 端使用 Android Keystore AES-GCM 加密业务 Token、主标识、登录标识列表、微信绑定状态、昵称和 HTTPS 头像 URL，并保持在账本备份、诊断、日志及截图之外；OpenID、UnionID、AppSecret、SMTP 凭据和微信 Token 不进入客户端 Session。
