...
- **无障碍服务自动观察内容**：用于支付完成后创建或补全待确认条目；属于可选功能，由用户完全掌控。在 Android 11 及以上版本，受支持支付结果界面的无障碍节点可与一次性本机临时截图 OCR 按字段融合，不以通知存在为前提。通知、无障碍节点和 OCR 原文仅作为对应待确认条目的本地证据分区保存。
...
- 受支持支付结果界面可进行一次临时截图以供内置本机 OCR 使用，并与当前无障碍节点按字段融合。Android 14 及以上仅捕获活动应用窗口；Android 11-13 使用屏幕截图 API。图片绝不持久化、绝不上传。支付结果页的通知、无障碍节点和 OCR 原文可保存在对应待确认条目的本地证据中，不上传账本同步服务；当用户单独开启敏感诊断日志时，接受的支付界面或活动手动导入会话中的文本还可加密保留在本地诊断存储中。
...
- 将本地 OCR 限制在 Android 11 及以上版本：自动路径只处理受支持支付结果界面，并把当次无障碍节点与瞬时 OCR 作为同一捕获批次的可选证据；手动路径只在用户主动开启的微信补录会话内处理当前账单详情。自动路径持久化前必须具备支付完成和无歧义货币证据，不要求通知存在；手动路径必须命中“当前状态 + 支付成功”“当前状态 + 对方已收”或“退款状态 + 已退款”任一完整签名，并包含无歧义金额。识别后立即释放截图；自动支付结果页 OCR 原文可进入对应待确认证据，手动补录 OCR 原文仍不进入账本数据库。单独开启诊断后，接受边界内的 OCR 文本还可进入加密诊断存储。
