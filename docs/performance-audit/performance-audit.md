...
- 批次 18 [PERF-A02]：待开始。先选一个能同时降低两侧转场页面组合、测量或 View 绘制的最小源码候选；只接受同进程首次/第二次 None/Profile 各 10 次均不退化的改动，再用批次 17 runtime tracing 复核受影响的子树。
...
- `perfetto-trace-analysis`：适用；批次 6 的 60 个 Trace、批次 10 的 3 条策略 Trace、批次 13 的 40 条 Trace、批次 14 的 40 条 Trace、批次 15 的 4 组导航用例以及批次 17 的 10 条 runtime-tracing Trace 均保留 evidence scratchpad 或聚合证据。批次 14–15 与 17 先使用 Macrobenchmark FrameTiming 高层汇总；当前 trace_processor 的 `android_jank` metric 不可用，随后以 Frame Timeline、线程状态、CPU frequency、Binder、memory/GC、power rail 和全局 D-state SQL 复核。设备 power rail 空包已明确记录为限制。
- `perfetto-sql`：适用；每个使用的表/视图先在 `perfetto-stdlib.md` 确认 schema，查询使用 upid/utid、GLOB、重叠时间窗和 `dur=-1` 处理。批次 14–15 和批次 17 使用 `android_frames`、`thread_slice`、`thread_state`、`android_binder_txns`、`android_monitor_contention_chain`、`android_garbage_collection_events`、`cpu_frequency_counters`、`memory_rss_and_swap_per_process`、`android_gpu_memory_per_process` 和 `android_power_rails_counters` 确认同进程首/次导航差异，并排除 I/O、Binder、锁、GPU cache miss 或调度阻塞是第二次导航 UI 长帧的共同主因。
