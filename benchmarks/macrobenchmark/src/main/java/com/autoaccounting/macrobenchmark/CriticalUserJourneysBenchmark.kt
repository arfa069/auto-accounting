...
class CriticalUserJourneysBenchmark {
    ...
    @Test
    fun automaticBookkeepingFirstNavigation() = measureAutomaticBookkeepingFirstNavigation(
        CompilationMode.None()
    )

    @Test
    fun automaticBookkeepingFirstNavigationBaselineProfile() =
        measureAutomaticBookkeepingFirstNavigation(
            CompilationMode.Partial(BaselineProfileMode.Require)
        )

    private fun measureAutomaticBookkeepingFirstNavigation(
        compilationMode: CompilationMode
    ) = benchmarkRule.measureRepeated(
        packageName = TARGET_PACKAGE,
        metrics = listOf(FrameTimingMetric()),
    ...
    @Test
    fun automaticBookkeepingSecondNavigation() = measureAutomaticBookkeepingSecondNavigation(
        CompilationMode.None()
    )

    @Test
    fun automaticBookkeepingSecondNavigationBaselineProfile() =
        measureAutomaticBookkeepingSecondNavigation(
            CompilationMode.Partial(BaselineProfileMode.Require)
        )

    private fun measureAutomaticBookkeepingSecondNavigation(
        compilationMode: CompilationMode
    ) = benchmarkRule.measureRepeated(
        packageName = TARGET_PACKAGE,
        metrics = listOf(FrameTimingMetric()),
        ...
        setupBlock = {
            ...
            app.waitForHome()
            app.openAutomaticBookkeeping()
            app.returnHomeFromAutomaticBookkeeping()
        }
    ) {
        app.openAutomaticBookkeeping()
    }
