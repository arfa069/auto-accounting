...
class WechatLoginController(
    ...
    private fun validIdentifier(requireContact: Boolean): Boolean {
        val parsed = runCatching { com.bks.api.AccountIdentifierParser.parse(state.phone) }.getOrNull()
        if (parsed != null && (!requireContact || parsed.type != com.bks.api.AccountIdentifierTypeContract.USERNAME)) {
            return true
        }
