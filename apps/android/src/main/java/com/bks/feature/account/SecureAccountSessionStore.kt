    ...
    private fun ByteBuffer.decodeVersionThreeToFive(
        ...
        val primaryIdentifier = if (primaryTypeStr != null && primaryValStr != null) {
            com.bks.api.AccountIdentifierContract(
                type = com.bks.api.AccountIdentifierTypeContract.valueOf(primaryTypeStr),
                value = primaryValStr
            )
        } else null
        ...
        val idents = ArrayList<com.bks.api.AccountIdentifierContract>(identsCount)
        repeat(identsCount) {
            ...
            idents.add(
                com.bks.api.AccountIdentifierContract(
                    type = com.bks.api.AccountIdentifierTypeContract.valueOf(tStr),
                    value = vStr,
                    verified = verified
                )
            )
...
internal class AndroidKeystoreAccountSessionCipher : AccountSessionCipher {
    ...
    private companion object {
        ...
        const val KEY_ALIAS = "bks_account_session_v1"
        const val TRANSFORMATION = "AES/GCM/NoPadding"
        val AUTHENTICATED_DATA = "bks-account-session-v1".toByteArray(Charsets.UTF_8)
    }
}
