...
import com.bks.api.AccountIdentifierParser
...
internal fun UnlinkWechatDialog(
    state: AccountIdentityUiState,
    availableIdentifiers: List<com.bks.api.AccountIdentifierContract>,
    onStateChange: (AccountIdentityUiState) -> Unit,
    onRequestSms: () -> Unit,
    onConfirm: () -> Unit
) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            availableIdentifiers.forEach { identifier ->
                                val label = when (identifier.type) {
                                    com.bks.api.AccountIdentifierTypeContract.PHONE -> "手机验证码"
                                    com.bks.api.AccountIdentifierTypeContract.EMAIL -> "邮箱验证码"
                                    else -> return@forEach
                                }
...
internal fun String.isValidIdentityContact(): Boolean = runCatching {
    val type = com.bks.api.AccountIdentifierParser.parse(this).type
    type == com.bks.api.AccountIdentifierTypeContract.PHONE ||
        type == com.bks.api.AccountIdentifierTypeContract.EMAIL
}.getOrDefault(false)
...
internal fun List<com.bks.api.AccountIdentifierContract>.identitySummary(): String? {
    val identifier = firstOrNull() ?: return null
    return when (identifier.type) {
        com.bks.api.AccountIdentifierTypeContract.PHONE -> identifier.value.maskPhoneForIdentity()
        com.bks.api.AccountIdentifierTypeContract.EMAIL -> identifier.value.maskEmailForIdentity()
        com.bks.api.AccountIdentifierTypeContract.USERNAME -> identifier.value
    }
}
