...
internal fun Map<String, String>.aiProviderRuntimeConfig(
    ...
    val apiKey = this["BKS_AI_API_KEY"].orEmpty().trim()
    val model = this["BKS_AI_MODEL"].orEmpty().trim()
    val endpoint = this["BKS_AI_ENDPOINT"]
        ?.trim()
        ?.takeIf(String::isNotBlank)
        ?.let { runCatching { URI(it) }.getOrNull() }
    val authStyle = parseAuthStyle(this["BKS_AI_AUTH_STYLE"], defaultAuthStyle)
    val outputMode = parseOutputMode(this["BKS_AI_OUTPUT_MODE"], defaultOutputMode)
    val reasoningMode = parseReasoningMode(this["BKS_AI_REASONING_MODE"])
    val apiVersion = this["BKS_AI_API_VERSION"]
        ?.trim()
        ?.takeIf(String::isNotBlank)
        ?: DEFAULT_API_VERSION
    val connectTimeout = positiveTimeout(
        "BKS_AI_CONNECT_TIMEOUT_MILLIS",
        DEFAULT_CONNECT_TIMEOUT_MILLIS
    )
    val readTimeout = positiveTimeout(
        "BKS_AI_READ_TIMEOUT_MILLIS",
