...
class SmtpEmailProvider internal constructor(
    ...
    companion object {
        ...
        fun fromEnvironment(env: Map<String, String> = System.getenv()): EmailProvider {
            if (!env["BKS_EMAIL_PROVIDER"].orEmpty().equals("smtp", ignoreCase = true)) {
                return MissingEmailProvider
            }

            val host = env["BKS_SMTP_HOST"].orEmpty()
            val security = env["BKS_SMTP_SECURITY"].orEmpty()
                .ifBlank { SMTP_SECURITY_STARTTLS }
                .lowercase()
            if (host.isBlank() || security !in setOf(SMTP_SECURITY_STARTTLS, SMTP_SECURITY_SSL)) {
                return MissingEmailProvider
            }
            val port = env["BKS_SMTP_PORT"]?.toIntOrNull()
                ?: if (security == SMTP_SECURITY_SSL) 465 else 587
            val username = env["BKS_SMTP_USERNAME"].orEmpty()
            val password = env["BKS_SMTP_PASSWORD"].orEmpty()
            val fromAddress = env["BKS_SMTP_FROM"].orEmpty().ifBlank { username }
