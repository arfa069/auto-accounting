...
class SmsProviderTest {
    ...
    fun fromEnvironmentParsesWebhookProvider() {
        val provider = SmsProvider.fromEnvironment(
            mapOf(
                "AUTO_ACCOUNTING_SMS_PROVIDER" to "webhook",
                "AUTO_ACCOUNTING_SMS_WEBHOOK_URL" to "https://example.com/webhook",
                "AUTO_ACCOUNTING_SMS_API_KEY" to "secret-key"
            )
        )
    ...
    fun fromEnvironmentParsesAliyunPnvsProvider() {
        val provider = SmsProvider.fromEnvironment(
            mapOf(
                "AUTO_ACCOUNTING_SMS_PROVIDER" to "aliyun_pnvs",
                "AUTO_ACCOUNTING_SMS_ALIYUN_ACCESS_KEY_ID" to "LTAI5tExampleKeyId",
                "AUTO_ACCOUNTING_SMS_ALIYUN_ACCESS_KEY_SECRET" to "ExampleAccessKeySecret123456",
                "AUTO_ACCOUNTING_SMS_SIGN_NAME" to "系统预置签名",
                "AUTO_ACCOUNTING_SMS_TEMPLATE_CODE" to "SMS_12345678"
            )
        )
    ...
    fun fromEnvironmentReturnsMissingSmsProviderWhenAliyunCredentialsIncomplete() {
        val provider = SmsProvider.fromEnvironment(
            mapOf(
                "AUTO_ACCOUNTING_SMS_PROVIDER" to "aliyun",
                "AUTO_ACCOUNTING_SMS_ALIYUN_ACCESS_KEY_ID" to "LTAI5tExampleKeyId"
    ...
    fun fromEnvironmentReturnsMissingSmsProviderWhenProviderUnsetOrUnknown() {
        ...
        assertEquals(MissingSmsProvider, SmsProvider.fromEnvironment(mapOf("AUTO_ACCOUNTING_SMS_PROVIDER" to "unknown")))
    }
